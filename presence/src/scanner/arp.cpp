...
{
	return (char *)libnet_getdevice(this->handle);
}
...
void Arp::getInfo()
{
	...
	char *device;
	device = (char*)libnet_getdevice(this->handle);
	if (device)
	{
		cout.flags(ios::left);
...
libnet_ptag_t Arp::build_packet(libnet_ether_addr *srcPhAddr,
	u_int32_t srcProAddr,
	libnet_ether_addr *dstPhAddr,
	u_int32_t dstProAddr,
	uint16_t op)
{
